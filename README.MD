# Dos's Reconstruction of ZZT Fork

I am just playing around here and have no plans to actual create a serious fork. If you are interested in this code, please look to the original.

## Changelist

 * Water placed via the default editor now matches the colors used for water in DEMO.ZZT,
 * Keys on the HUD that the player does not have are now drawn in black-on-dark-blue, making a key silhouette
 * Board exits are listed beneath keys

# The Reconstruction of ZZT

The Reconstruction of ZZT is a reverse-engineered recreation of the source code to the last official release of ZZT,
ZZT 3.2. The output executable is byte-for-byte identical with said release, which means that the source code accurately
represents the original engine's behavior.

The intent behind this project is to faciliate improved preservation and accessibility of ZZT's worlds and community,
as well as faciliate new, exciting developments.
...
* **MISC** - miscellaneous files which are not part of the build process:
    * **relocfix.py** - Python 3 script for fixing relocation table semgents in EXE binaries after LZEXE decompression,
